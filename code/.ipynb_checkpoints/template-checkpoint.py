...
def rhs_numba(t, y, params_array, I_dict, Pool1, Pool2, del1, del2, del3, del4):
    # 解包参数（Numba不支持字典，需转为数组）
    (k41, KK41, k51, k52, k53, k54, kpq, pot1, pot2, pot3, 
     kp680, tauc, kcar, kda, cm, d, tauhp, tauhp1, tauhn, taupt, taupt1) = params_array
...
def reaction_system(params):
    ...
    params_array = np.array([
        params['k41'], params['KK41'], params['k51'], params['k52'], 
        params['k53'], params['k54'], params['kpq'], params['pot1'],
        params['pot2'], params['pot3'], params['kp680'], params['tauc'],
        params['kcar'], params['kda'], params['cm'], params['d'],
        params['tauhp'], params['tauhp1'], params['tauhn'], 
        params['taupt'], params['taupt1']
    ])
    ...
    def rhs(t, y):
        return rhs_numba(t, y, params_array, I_static, 
                         Pool1, Pool2, del1, del2, del3, del4)
    return rhs
...
def array_to_tuple(arr):
    return tuple(arr.round(6)) 

@lru_cache(maxsize=100)  # 缓存最近100次计算结果
def cached_simulate(u_tuple):
    u = np.array(u_tuple)
...
def loss_sum(u):
    try:
        u_tuple = array_to_tuple(u)  # 转换格式
        FLY = cached_simulate(u_tuple)  # 使用缓存版本
