# 第二版，主要针对计算时间进行优化，同时优化取值范围（del1-del4）使其更加合理，切换了计算方法从BDF到LSODA。
# 添加了计时功能 time 
# 第三次运行 增加部分计算
import numpy as np
...
def simulate_FLY_from_u(u):
    ...
    if call_count % 10 == 0:
        print(f"已调用 {call_count} 次，用时 {total_time:.2f}s，预计剩余 {avg_time*(500-call_count)/60:.1f} 分钟")
...
if __name__ == '__main__':
    ...
    # 保存图像并显示
    plt.savefig('plot/comparison_plot_logx_y1_opt_3.png', dpi=300, bbox_inches='tight')
    plt.show()
    df = pd.DataFrame.from_dict(p_opt, orient='index', columns=['Value'])
    df.to_csv('csv/parameters_y1_opt_3.csv', float_format='%.4g')  # 保存CSV
